...
(async () => {
  try {
    ...
    if (config.slack.oocChannelId) {
      await app.client.conversations.join({ channel: config.slack.oocChannelId }).catch((err: unknown) => {
        if ((err as { data?: { error?: string } })?.data?.error === 'already_in_channel') return;
